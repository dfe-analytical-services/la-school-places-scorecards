...
summary_stats_3yr <- forecast_summary_stats(
  dfScorecardsAll, 3,
  params$input_phase_choice
)
```
...
forecast_accuracy_3yr <- dfScorecardArea %>%
  filter(name == "For_3") %>%
  pull(value) %>%
  roundFiveUp(., 3) #* 100

label_desc_3yr <- forecast_accuracy_desclabel(
  forecast_accuracy_3yr,
  params$input_la_choice,
  summary_stats_3yr
)
threeyearlabel <-
  if (label_desc_3yr != "accurate") {
    paste0("Three years ahead: ", format_perc(forecast_accuracy_3yr, output = "latex"))
  } else {
    paste0("<b>Three years ahead: ")
  }
```
...
``` {r forecast23,echo = FALSE, warning = FALSE, message=FALSE, results='asis', fig.height=1,fig.show="hold", out.width="50%"}
plot_forecast(
  dfScorecardArea,
  dfScorecardsAll,
  params$input_la_choice,
...
facc_table3 <- as.data.frame(
  summary_stats_3yr %>% mutate(accuracy = format_perc(accuracy)) %>%
    pivot_wider(names_from = quantile, values_from = accuracy)
)
...
cat("\\footnotesize ")
cat("\\centering ")
print(knitr::kable(facc_table3, "latex", booktabs = T))
cat("\\end{tabular}")
```
