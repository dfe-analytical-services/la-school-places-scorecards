...
Forecast accuracy of pupil projections for 21/22, made one year and three years previously


``` {r forecastacc, echo = FALSE, fig.show="hold", out.width = "100%",crop=TRUE}
...
``` {r forecast1,echo = FALSE, warning = FALSE, message=FALSE, results='asis'}
    ...
    range_values$accuracy[1] <- (ceiling(abs(range_values$accuracy[1])) * range_values$accuracy[1] / abs(range_values$accuracy[1]))
     ggplot(
      ...
      geom_bar(stat = "identity", width = 100, height=10) +
...
``` {r forecast2,echo = FALSE, warning = FALSE, message=FALSE, results='asis'}
 

     forecast_accuracy3 <- live_scorecard_data %>% 
       filter(name == "For_3") 

     forecast_accuracy3$value <- forecast_accuracy3$value %>% roundFiveUp(., 3) * 100 
     ...
     p <- ggplot(forecast_accuracy3, aes(name, value, fill = value, text = paste0(params$input_la_choice, ": ", value, "%"))) + 
     ...
     #  geom_hline(yintercept = 100. * (forecast_range %>% filter(LA_name == "England"))$value) + 
       geom_hline(yintercept = range_values$accuracy[4], color="Grey", linetype = "dashed") + 
       geom_hline(yintercept = forecast_accuracy3$value, size = 1.) + 
       labs(x = "", y = "Accuracy (%)") + 
       coord_flip() 
