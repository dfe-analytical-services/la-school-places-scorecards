...
total_funding <- scorecards_data %>%
  ...
  mutate(
    Funding =
      ifelse(params$input_la_choice == "England", round_five_up(Funding / 1000000000, 2),
        round_five_up(Funding / 1000000, 0)
      )
  ) %>%
  as.numeric()
...
dfTopLine <- data.frame(
  funding = ifelse(params$input_la_choice == "England",
    paste0(total_funding, "billion"),
    paste0(total_funding, "million")
  ),
  empty = ""
)
colnames(dfTopLine) <- c(
  paste0("Total primary and secondary basic need funding ", funding_year), " "
)
```

```{r growth in pupil number, echo = FALSE}
# Display growth in pupil numbers
...
anticipated_growth_perc <- dfScorecardArea %>%
  filter(name == "Angro") %>%
  pull(value) # %>%
# round_five_up(., 3) * 100

anticipated_growth_perc1 <- format_perc(anticipated_growth_perc)

textTopLineGrowth <- paste0("Growth in ", str_to_lower(params$input_phase_choice), " pupil numbers 2009/10 to ", next_year, ": ", format_perc(growth_perc), "%")

textTopLineAnticipatedGrowth <- paste0("Anticipated growth in ", str_to_lower(params$input_phase_choice), " pupil numbers ", next_year, " to ", plan_year, ": ", format_perc(anticipated_growth_perc), "%")
```


```{r unfilled places, echo = FALSE}
# Display growth in pupil numbers


unfilled_places_perc <- dfScorecardArea %>%
  filter(name == "QuanUP") %>%
  pull(value) %>%
  round_five_up(., 1)

textTopLineunfilledplaces <- paste0("Current percentage of unfilled ", str_to_lower(params$input_phase_choice), " places ", this_year, ": ", unfilled_places_perc, "%")
```
...
`r format_perc(growth_perc,output = "latex")`
\tcblower
Change in `r str_to_lower(params$input_phase_choice)` pupil numbers 2009/10 to `r next_year`
\end{tcolorbox}
...
`r unfilled_places_perc`\%
\tcblower
Current percentage of unfilled `r str_to_lower(params$input_phase_choice)` places `r this_year`
\end{tcolorbox}
...
`r format_perc(anticipated_growth_perc,output = "latex")`
\tcblower
Anticipated change in `r str_to_lower(params$input_phase_choice)` pupil numbers `r next_year` to `r plan_year`
\end{tcolorbox}
...
``` {r quantity_b, echo = FALSE, warning = FALSE, fig.show="hold", out.width = "100%",crop=TRUE}
...
plot_ly(
  ...
  type = "bar", name = paste0("Total places created between May 2010 and May ", SCAP_ref),
  text = ~ scales::comma(QuanIn), textposition = "inside", textfont = list(color = "#FFF", width = 0.2)
) %>%
  add_trace(y = ~QuanPP, marker = list(color = c("#3182bd")), name = paste0("New places planned for delivery between May ", SCAP_ref, " and September ", plannedplaces_ref), text = ~ scales::comma(QuanPP), textposition = "outside", cliponaxis = FALSE, textfont = list(color = "#000"), width = 0.2) %>%
  add_trace(y = ~QuanRP, marker = list(color = c("#6baed6")), name = paste0("Estimated additional places still needed to meet demand in ", plan_year), text = ~ scales::comma(QuanRP), textposition = "outside", cliponaxis = FALSE, textfont = list(color = "#000"), width = 0.2) %>%
  layout(
    yaxis = list(title = ""),
    xaxis = list(title = ""),
    barmode = "bar",
...
The starting point is 0, an accurate score, indicated by a thin vertical line.
...
``` {r forecast_setup,echo = FALSE, warning = FALSE, message=FALSE, results='asis'}
summary_stats_1yr <- forecast_summary_stats(
  dfScorecardsAll, 1,
  params$input_phase_choice
)
summary_stats_2yr <- forecast_summary_stats(
  dfScorecardsAll, 2,
  params$input_phase_choice
)
```
...
forecast_accuracy_1yr <- dfScorecardArea %>%
  filter(name == "For_1") %>%
  pull(value) %>%
  round_five_up(., 3) #* 100

label_desc_1yr <- forecast_accuracy_desclabel(
  forecast_accuracy_1yr,
  params$input_la_choice,
  summary_stats_1yr
)
oneyearlabel <-
  if (label_desc_1yr != "accurate") {
    paste0("One year ahead: ", format_perc(forecast_accuracy_1yr, output = "latex"))
  } else {
    paste0("<b>One year ahead: ")
  }

forecast_accuracy_2yr <- dfScorecardArea %>%
  filter(name == "For_2") %>%
  pull(value) %>%
  round_five_up(., 3) #* 100

label_desc_2yr <- forecast_accuracy_desclabel(
  forecast_accuracy_2yr,
  params$input_la_choice,
  summary_stats_2yr
)
threeyearlabel <-
  if (label_desc_2yr != "accurate") {
    paste0("Two years ahead: ", format_perc(forecast_accuracy_2yr, output = "latex"))
  } else {
    paste0("<b>Two years ahead: ")
  }
```


``` {r forecast_statshead,echo = FALSE, warning = FALSE, message=FALSE, results='asis'}
cat("\\begin{tabular}{p{0.48\\linewidth} p{0.48\\linewidth} }")
cat("\\bf\\large\\color{dfeheadingblue} ")
cat(oneyearlabel)
cat("\\ & \\bf\\large\\color{dfeheadingblue} ")
cat(threeyearlabel)
cat("\\ \\\\")
...
``` {r forecast22,echo = FALSE, warning = FALSE, message=FALSE, results='asis', fig.height=1,fig.show="hold", out.width="50%"}
plot_forecast(
  dfScorecardArea,
  dfScorecardsAll,
  params$input_la_choice,
  params$input_phase_choice, 1
)
plot_forecast(
  dfScorecardArea,
  dfScorecardsAll,
  params$input_la_choice,
  params$input_phase_choice, 2
)
```


``` {r forecast4,echo = FALSE, warning = FALSE, message=FALSE, results='asis'}
facc_table1 <- as.data.frame(
  summary_stats_1yr %>% mutate(accuracy = format_perc(accuracy)) %>%
    pivot_wider(names_from = quantile, values_from = accuracy)
)
facc_table2 <- as.data.frame(
  summary_stats_2yr %>% mutate(accuracy = format_perc(accuracy)) %>%
    pivot_wider(names_from = quantile, values_from = accuracy)
)
...
cat("\\footnotesize ")
cat("\\centering ")
print(knitr::kable(facc_table1, "latex", booktabs = T))
cat("&")
cat("\\footnotesize ")
cat("\\centering ")
print(knitr::kable(facc_table2, "latex", booktabs = T))
cat("\\end{tabular}")
```
...
## Preference
### Proportion of applicants who received an offer of one of their top three preference schools for September `r preference_next_year` entry

```{r preference_text_NY, echo = FALSE, message=FALSE}
dfScorecardArea_all_la <- scorecards_data_pivot %>% filter(Phase == params$input_phase_choice)


# Take filtered data, search for growth rate, pull the value and tidy the number up
PrefT3_NY_E <- dfScorecardArea_all_la %>%
...
```
### Proportion of applicants who received an offer of one of their top three preference schools for September `r preference_current_year` entry

```{r preference_text_CY, echo = FALSE, message=FALSE}
dfScorecardArea_all_la <- scorecards_data_pivot %>% filter(Phase == params$input_phase_choice)


# Take filtered data, search for growth rate, pull the value and tidy the number up
PrefT3_CY_E <- dfScorecardArea_all_la %>%
...
```{r preference_barchart, echo = FALSE, message=FALSE, fig.width=8, fig.height=4.2, crop=TRUE}
...
preference_data <- preference_data %>%
  ...
  mutate(
    ...
    # Neaten up percs
    value = as.numeric(round_five_up(value, 1)),
...
england_comp <-
  if (params$input_quality_chart == "Ofsted") {
    ...
    round_five_up(numerator / denominator * 100, 1)
  } else if (params$input_quality_chart == "Progress 8") {
    numerator <- dfScorecardArea_all_la %>%
      filter(LA_name == "England" &
...
``` {r quality_chart, fig.width=8, fig.height=4.2, crop=TRUE}           
# Scorecard data, filtered on user input AND including England as a comparison
dfScorecardArea_england_comp <- scorecards_data_pivot %>%
  filter(
    LA_name %in% c(params$input_la_choice, "England"),
...
ofsted_data <- dfScorecardArea_england_comp %>%
  ...
  mutate(
    places = if_else(value == 0, NA_integer_, as.integer(round_five_up(value, 0)))
  ) %>%
...
progress_8_data <- dfScorecardArea_england_comp %>%
  ...
  # Create new variable called places, replace 0s with NAs so it plots neatly
  mutate(places = if_else(value == 0, NA_integer_, as.integer(round_five_up(value, 0)))) %>%
  # Give NA for label if it's too small
  group_by(LA_name, place_type) %>%
  mutate(
    places_perc = places / sum(places, na.rm = TRUE),
...
perm_fig <- dfScorecardArea_all_la %>%
...
temp_fig <- dfScorecardArea_all_la %>%
  # Filter for Cost, places and project data
  filter(str_detect(name, "Cost|Places|Projects")) %>%
  # Create new column called data_type, based on the name of the data
  mutate(data_type = case_when(
...
new_fig <- dfScorecardArea_all_la %>%
  # Filter for Cost, places and project data
  filter(str_detect(name, "Cost|Places|Projects")) %>%
  # Create new column called data_type, based on the name of the data
  mutate(data_type = case_when(
...
cost_table <-
  ...
  mutate(
    cost_per_place = round_five_up(Cost / Place, 0),
